...
async function loadNumbersFromAPI() {
    try {
        var _ts = new Date().getTime(); var res = await fetch('data/numbers.json?t=' + _ts);
...
async function loadOtpsFromAPI() {
    ...
    try {
        var _ts2 = new Date().getTime(); var res = await fetch('data/otps/' + activePurchase.number + '.json?t=' + _ts2);
        if (!res.ok) {
            allOtps = [];
            return;
        }
